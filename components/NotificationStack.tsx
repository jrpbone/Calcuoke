...
const NotificationStack: React.FC<NotificationStackProps> = ({ notifications }) => (
  <div className="fixed top-6 right-6 z-[300] flex flex-col gap-3 pointer-events-none w-80">
    {notifications.map((notification) => (
      <div
        key={notification.id}
        className={`pointer-events-auto flex items-center gap-4 p-5 rounded-2xl border-l-[6px] backdrop-blur-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] transition-all ${notification.isExiting ? 'notification-exit' : 'notification-enter'} ${notification.type === 'success' ? 'bg-emerald-900/90 border-emerald-400' : 'bg-red-900/90 border-red-400'}`}
      >
